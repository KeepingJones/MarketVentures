...
Logic mirrors what a market data analyst actually does when investigating
a Bloomberg vs internal price discrepancy — work through the most common
...
def classify_break(
    ...
    # 4. FX rate break — affects entire non-base-currency book; always CRITICAL
    if asset_class == "fx":
    ...
    # 7. Moderate break — classify by asset class
    if asset_class in ("govt_bond", "corp_bond"):
        # Bond prices can legitimately differ when one source uses dirty price
        if diff_pct < 2.0:
